...
public class VisualizationModule {
    ...
    enum Status { IDLE, RUNNING, WAITING, LOCKED }
    ...
    static class CompletedBar {
        ...
        CompletedBar(int thread, int taskId,
            ...
            this.thread = thread; this.taskId = taskId;
            this.start  = start;  this.end    = end;
            this.locked = locked; this.color  = color;
        }
    }
    ...
    static class VisualizationPanel extends JPanel {
        ...
        JPanel buildControls() {
            ...
            p.add(tskSlider); p.add(tskVal);

            p.add(vsep());

            // legend
            p.add(dot(C_ACCENT));  p.add(lbl("Working"));
            p.add(dot(C_RED));     p.add(lbl("Locked"));
            p.add(dot(C_YELLOW));  p.add(lbl("Waiting"));
        ...
        JPanel dot(Color c) {
            return new JPanel() {
                { setBackground(C_PANEL); setPreferredSize(new Dimension(12, 18)); }
        ...
        void runTask(TaskSpec task) {
            ...
            for (int c = 0; c < chunks; c++) {
                try { Thread.sleep(task.duration / chunks); }
                catch (InterruptedException ignored) {}
        ...
        void log(String msg) {
            if (SwingUtilities.isEventDispatchThread()) {
                events.add(msg); logPanel.repaint();
            } else {
                SwingUtilities.invokeLater(() -> { events.add(msg); logPanel.repaint(); });
            }
        }
        ...
        class GanttPanel extends JPanel {
            ...
            void paintLive(Graphics2D g2, ThreadInfo ti, int idx,
                ...
                g2.setColor(fill); fillRR(g2, lx, ly, lw, lh);
                g2.setColor(edge);
                g2.setStroke(new BasicStroke(0.8f)); drawRR(g2, lx, ly, lw, lh);
                g2.setStroke(new BasicStroke(1));

                // progress stripe
                int sy = ly + lh - 3;
                g2.setColor(C_SURFACE); g2.fillRect(lx, sy, lw, 3);
                g2.setColor(edge);      g2.fillRect(lx, sy, (int)(lw * ti.progress), 3);
            }
            ...
            void paintWait(Graphics2D g2, ThreadInfo ti, int idx,
                ...
                g2.setColor(alpha(C_YELLOW, 55)); fillRR(g2, lx, ly, lw, lh);
            ...
            void fillRR(Graphics2D g, int x, int y, int w, int h) { g.fillRoundRect(x,y,w,h,5,5); }
            void drawRR(Graphics2D g, int x, int y, int w, int h) { g.drawRoundRect(x,y,w,h,5,5); }
            Color alpha(Color c, int a) { return new Color(c.getRed(),c.getGreen(),c.getBlue(),a); }
        ...
        class LogPanel extends JPanel {
            ...
            Color lineColor(String s) {
                if (s.startsWith("✅") || s.startsWith("✔")) return C_GREEN;
                if (s.startsWith("🔒") || s.startsWith("🔓")
                 || s.startsWith("🟡"))                       return C_YELLOW;
                if (s.startsWith("▶"))                        return C_ACCENT;
                return C_TEXT;
            }
